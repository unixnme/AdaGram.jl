...
WORKDIR /root
RUN apt-get update && apt-get install build-essential wget vim unzip git -y
...
RUN git clone https://github.com/sbos/AdaGram.jl.git
WORKDIR /root/AdaGram.jl
RUN sh build.sh
RUN /root/julia-1.0.5/bin/julia -e 'using Pkg; Pkg.add(PackageSpec(url="https://github.com/sbos/AdaGram.jl.git"))'
